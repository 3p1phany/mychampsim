...
void IsbPrefetcher::isb_train_addr(uint64_t pc, bool str_addr_exists, uint64_t addr_B, uint32_t str_addr_B)
{
    ...
    if (training_unit.access(pc, addr_A, str_addr_A, addr_B))
    {
        ...
        uint32_t cnt_str_addr_B_onchip, cnt_str_addr_A_offchip, cnt_str_addr_B_offchip;
        bool addr_B_str_onchip_exists = on_chip_corr_matrix.get_structural_address(addr_B, cnt_str_addr_B_onchip, false);
        bool addr_B_str_offchip_exists = off_chip_corr_matrix.get_structural_address(addr_B, cnt_str_addr_B_offchip);
        bool addr_A_str_offchip_exists = off_chip_corr_matrix.get_structural_address(addr_B, cnt_str_addr_A_offchip);
        if(addr_B_str_onchip_exists){
            if ((cnt_str_addr_B_onchip >> STREAM_MAX_LENGTH_BITS) ==
        ...
        } else if(addr_B_str_offchip_exists && addr_A_str_offchip_exists) {
            if ((cnt_str_addr_B_offchip >> STREAM_MAX_LENGTH_BITS) ==
                    ((cnt_str_addr_A_offchip + 1)>>STREAM_MAX_LENGTH_BITS)){
                conf_inc++;
            } else {
                conf_dec++;
                if (load_identity[cpu].is_recursive(pc)) {
...
IsbPrefetcher::IsbPrefetcher(const pf_isb_conf_t *p, uint32_t cpu):
#ifndef OFF_CHIP_ONLY
...
void IsbPrefetcher::dump_stats()
{
    ...
    cout << "metadata_conf_change: " << conf_dec <<endl;
