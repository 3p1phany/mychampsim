...
void print_roi_stats(uint32_t cpu, CACHE* cache)
{
  ...
  if (TOTAL_ACCESS > 0) {
    ...
    cout << "  USEFUL: " << setw(10) << cache->pf_useful
         ...
         << "  LATE: " << setw(10) << cache->pf_late
         << "  FILL: " << setw(10) << cache->pf_fill
         << endl;
...
void print_roi_read_stats(uint32_t cpu, CACHE* cache)
{
  ...
  if (TOTAL_ACCESS > 0) {
    ...
    if (cache->NAME=="cpu0_L1D"){
      cout << "\nCollect Load Info:" <<endl;
      std::vector<uint64_t> load_pc;
      for(auto &[key, value]: cache->sim_load_info[cpu])
          load_pc.emplace_back(key);

      sort(load_pc.begin(), load_pc.end(), [&](const uint64_t&a, const uint64_t&b)->bool{
          return cache->sim_load_info[cpu][a].total_count==cache->sim_load_info[cpu][b].total_count? a<b : cache->sim_load_info[cpu][a].total_count > cache->sim_load_info[cpu][b].total_count;
      });
      ...
      for(auto i = load_pc.begin(); i != load_pc.end(); i++){
          uint64_t pc = *i;
          load_miss_info_t info_item =  cache->sim_load_info[cpu][*i];
