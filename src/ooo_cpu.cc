...
extern uint8_t MAX_INSTR_SOURCES;
...
void O3_CPU::init_instruction(ooo_model_instr arch_instr)
{
  ...
  for (int i = 0; i < MAX_INSTR_SOURCES; i++) {
    if(trace_type != TRACE_TYPE_LOONGARCH){
  ...
  if(trace_type == TRACE_TYPE_LOONGARCH){
    assert (arch_instr.op < LISA_ENDING);
    LA_OPCODE op = LA_OPCODE(arch_instr.op);
    assert (op_is_branch(op) == arch_instr.is_branch);
    ...
    } else if(op == LISA_JIRL && arch_instr.destination_registers[0] == REG_LOONGARCH_RA){
      // indirect call
      arch_instr.is_branch = 1;
      arch_instr.branch_taken = 1;
      arch_instr.branch_type = BRANCH_INDIRECT_CALL;
    } else if(op == LISA_JIRL && arch_instr.destination_registers[0] == 0 && arch_instr.source_registers[0] == 1 && arch_instr.source_reg_val[1] == 0){
      // return
      arch_instr.is_branch = 1;
      arch_instr.branch_taken = 1;
      arch_instr.branch_type = BRANCH_RETURN;
    } else if(op == LISA_JIRL && arch_instr.destination_registers[0] == 0){
      // indirect branch
      arch_instr.is_branch = 1;
      arch_instr.branch_taken = 1;
...
void O3_CPU::promote_to_decode()
{
  ...
  while (available_fetch_bandwidth > 0 && !IFETCH_BUFFER.empty() && !DECODE_BUFFER.full() && IFETCH_BUFFER.front().translated == COMPLETED
    ...
    if (IFETCH_BUFFER.front().decoded)
  ...
  if(warmup_complete[cpu] && available_fetch_bandwidth > 0 && !IFETCH_BUFFER.empty() && DECODE_BUFFER.full() && IFETCH_BUFFER.front().translated == COMPLETED
         && IFETCH_BUFFER.front().fetched == COMPLETED){
    DECODE_BUFFER_FULL++;
  }
...
void O3_CPU::decode_instruction()
{
  ...
  while (available_decode_bandwidth > 0 && DECODE_BUFFER.has_ready() && !DISPATCH_BUFFER.full()) {
    ...
    for(int i = 0; i < 2; i++){
        if(pt[cpu][db_entry.source_registers[i]].type!=NONE){
            is_depend[i] = true;
            depend_idx = i;
        }
    }
    ...
    //// Search IPT
    uint64_t trace_ip = (trace_type == TRACE_TYPE_LOONGARCH)? (db_entry.ip >> 2) : db_entry.ip;
    ...
    if(db_entry.destination_registers[0] != 0){
        ...
        IDM_OP op = decode_imd_op(db_entry.op, db_entry.source_reg_val[1], db_entry.source_reg_val[2]);
        //if(op == IDM_INVALID)
        //    cout << "NOTE: Unknown op! pc: " << hex << db_entry.ip << dec << endl;
        if(ip_stride_hit){
            if(!dct_hit){
                DCT_ITEM new_item(true, true, true, 3, trace_ip, op, true, 0, 0
                #ifdef PREFETCH_DEBUG
                    , db_entry.ip, current_cycle
                #endif
                );
        ...
        if(!ip_stride_hit && has_depend && !both_depend){
            ...
            if (op_is_load_ldptr((LA_OPCODE) db_entry.op)){
                new_src = (new_src << 2);
            }

            if(!dct_hit && prev_dct_hit){
                DCT_ITEM new_item(true, false, is_load, 1, trace_ip, op, undepend_idx, new_src, last_dct_ptr
            #ifdef PREFETCH_DEBUG
                , db_entry.ip, current_cycle
            #endif
                );
        ...
        if(ip_stride_hit && dct_hit_idx != -1) {
            pt[cpu][db_entry.destination_registers[0]].type = ORIGINAL_STRIDE;
            pt[cpu][db_entry.destination_registers[0]].dct_ptr = dct_hit_idx;
        } else if(has_depend && !both_depend && dct_hit_idx != -1) {
            pt[cpu][db_entry.destination_registers[0]].type = INFECTED_STRIDE;
            pt[cpu][db_entry.destination_registers[0]].dct_ptr = dct_hit_idx;
        } else {
            pt[cpu][db_entry.destination_registers[0]].type = NONE;
        }
        ...
        if(ip_stride_hit){
            ...
            complex_infect_info[cpu][db_entry.destination_registers[0]] = NONE;
        }
...
if(start_print){
  ...
  if(warmup_complete[cpu] && available_decode_bandwidth > 0 && DECODE_BUFFER.has_ready() && DISPATCH_BUFFER.full()){
...
void O3_CPU::dispatch_instruction()
{
  ...
  while (available_dispatch_bandwidth > 0 && DISPATCH_BUFFER.has_ready() && !ROB.full()) {
    /*
     * Note: Because we want gretch to detect the stride change, ChampSim will go to the beginning of the
     *       trace after warmup_complete. However, the trace maybe begin with B (for A[[B[i]] program). And
     *       B maybe blocked so it is issued after A. In that case, gretch will not be able to detech the
     *       memory pattern. So after warmup completion, we will clear the ROB.
     */
    if(DISPATCH_BUFFER.front().wait_rob_empty && !wait_rob_empty){
      wait_rob_empty = true;
    } else if(wait_rob_empty && ROB.empty()){
      wait_rob_empty = false;
    }
  ...
  if(warmup_complete[cpu] && available_dispatch_bandwidth > 0 && DISPATCH_BUFFER.has_ready() && ROB.full())
    ROB_FULL++;
...
int O3_CPU::prefetch_code_line(uint64_t pf_v_addr) { return static_cast<CACHE*>(L1I_bus.lower_level)->prefetch_line( pf_v_addr, FILL_L1, 0); }
...
void O3_CPU::schedule_instruction()
{
  ...
  for (auto rob_it = std::begin(ROB); rob_it != std::end(ROB) && search_bw > 0; ++rob_it) {
    if (rob_it->scheduled == 0) {
      ...
      #ifdef PRINT_TRACE
        cout << "[do_scheduling] cycles: " << current_cycle 
             << ", instr: " << rob_it->instr_id << hex
             << dec << endl;
      #endif
...
void O3_CPU::do_scheduling(champsim::circular_buffer<ooo_model_instr>::iterator rob_it)
{
  ...
  if (rob_it->is_memory)
    ...
    // ADD LATENCY
    rob_it->event_cycle = current_cycle +  SCHEDULING_LATENCY;
  }
}
...
void O3_CPU::execute_instruction()
{
  ...
  while (exec_issued < EXEC_WIDTH && !ready_to_execute.empty()) {
    do_execution(ready_to_execute.front());
      ...
      #ifdef PRINT_TRACE
        cout << "[do_execution] cycles: " << current_cycle 
             << ", instr: " << ready_to_execute.front()->instr_id << hex
             << dec << endl;
      #endif
...
void O3_CPU::do_execution(champsim::circular_buffer<ooo_model_instr>::iterator rob_it)
{
  ...
  rob_it->event_cycle = current_cycle +  EXEC_LATENCY;
...
void O3_CPU::schedule_memory_instruction()
{
  ...
  for (auto rob_it = std::begin(ROB); rob_it != std::end(ROB) && search_bw > 0; ++rob_it) {
    if (rob_it->is_memory && rob_it->num_reg_dependent == 0 && (rob_it->scheduled == INFLIGHT)){
      ...
      #ifdef PRINT_TRACE
        cout << "[do_memory_scheduling] cycles: " << current_cycle 
             << ", instr: " << rob_it->instr_id << hex
             << dec << endl;
      #endif
    }
...
void O3_CPU::do_memory_scheduling(champsim::circular_buffer<ooo_model_instr>::iterator rob_it)
{
  ...
  // load
  for (uint32_t i = 0; i < MAX_INSTR_SOURCES; i++) {
...
void O3_CPU::add_store_queue(champsim::circular_buffer<ooo_model_instr>::iterator rob_it, uint32_t data_index)
{
  ...
  sq_it->size = rob_it->ls_size;
  sq_it->wdata = rob_it->source_reg_val[0];
...
void O3_CPU::operate_lsq()
{
  ...
  while (load_issued < LQ_WIDTH && !RTL0.empty()) {
    ...
    #ifdef PRINT_TRACE
        cout << "[RTL0.pop ] cycles: " << current_cycle 
             << ", instr: " << RTL0.front()->instr_id << hex
             << ", vaddr: " << RTL0.front()->virtual_address 
             << dec << endl;
    #endif
...
//   load_issued = 0;
  while (load_issued < LQ_WIDTH && !RTL1.empty()) {
    ...
    #ifdef PRINT_TRACE
        cout << "[RTL1.pop ] cycles: " << current_cycle 
             << ", instr: " << RTL1.front()->instr_id << hex
             << ", vaddr: " << RTL1.front()->virtual_address 
             << dec << endl;
    #endif
...
void O3_CPU::execute_store(std::vector<LSQ_ENTRY>::iterator sq_it)
{
  ...
  for (auto dependent : sq_it->rob_index->memory_instrs_depend_on_me) {
    ...
    for (uint32_t j = 0; j < MAX_INSTR_SOURCES; j++) { // which one is dependent?
...
int O3_CPU::execute_load(std::vector<LSQ_ENTRY>::iterator lq_it)
{
  ...
  data_packet.lq_index_depend_on_me = {lq_it};
  data_packet.pf_metadata = l1_encode_metadata(lq_it->rob_index[0].destination_registers[0],
                                               lq_it->size, lq_it->unsign_ext);
  data_packet.size = lq_it->size;
  data_packet.unsign_ext = lq_it->unsign_ext;
...
void O3_CPU::do_complete_execution(champsim::circular_buffer<ooo_model_instr>::iterator rob_it)
{
  ...
  #ifdef ENABLE_DBP
  if(op_is_load((LA_OPCODE)rob_it->op)){
    // add possible producer's retval to FIFO
    dbp_load_ret[cpu].insert(rob_it->ip, rob_it->ret_val, rob_it->ls_size);
    uint64_t offset[64];
    uint64_t cnt = dbp_load_identity[cpu].check(rob_it->ip, offset);
    for (uint64_t i = 0; i < cnt; i++){
      dbp_prefq[cpu].add(rob_it->ret_val, offset[i]);
    }
  }
  #endif
  ...
  for (auto dependent : rob_it->registers_instrs_depend_on_me) {
    ...
    if (dependent->num_reg_dependent == 0) {
      if (dependent->is_memory) {
        ...
        #ifdef ENABLE_DBP
        if(op_is_load((LA_OPCODE)dependent->op)){
          // lookup FIFO to check if this load is a consumer
          // TODO: What if compiler put base addr to source_reg_val[1]?
          dbp_load_ret[cpu].lookup(dependent->ip, dependent->source_reg_val[0], dependent->source_reg_val[1]);
        }
        #endif
      }
      else {
...
void O3_CPU::complete_inflight_instruction()
{
  ...
  if ((inflight_reg_executions > 0) || (inflight_mem_executions > 0)) {
    ...
    while (rob_it != std::end(ROB) && complete_bw > 0) {
      ...
      #ifdef PRINT_TRACE
        cout << "[do_complete_execution] cycles: " << current_cycle 
             << ", instr: " << rob_it->instr_id << hex
             << dec << endl;
      #endif
...
void O3_CPU::handle_memory_return()
{
  ...
  while (to_read > 0 && !DTLB_bus.PROCESSED.empty()) { // DTLB
    ...
    for (auto lq_merged : dtlb_entry.lq_index_depend_on_me) {
      //if (current_cycle >= 82000000)
      //printf("%ld : %lx %lx %lx, %lx\n",current_cycle, &dtlb_entry, dtlb_entry.data, lq_merged, lq_merged->virtual_address);
      ...
      #ifdef PRINT_TRACE
          cout << "[RTL1.push] cycles: " << current_cycle 
               << ", instr: " << lq_merged->instr_id << hex
               << ", vaddr: " << lq_merged->virtual_address 
               << dec << endl;
      #endif
  ...
  while (to_read > 0 && !L1D_bus.PROCESSED.empty()) { // L1D
    ...
    for (auto merged : l1d_entry.lq_index_depend_on_me) {
      ...
      #ifdef PRINT_TRACE
          cout << "[L1D_bus.pop] cycles: " << current_cycle 
               << ", instr: " << merged->instr_id << hex
               << ", vaddr: " << merged->virtual_address 
               << dec << endl;
      #endif
...
void O3_CPU::retire_rob()
{
  ...
  if(!ROB.empty() && (ROB.front().executed != COMPLETED)){
    RETIRE_STALL_CYCLE++;
  }
  ...
  while (retire_bandwidth > 0 && !ROB.empty() && (ROB.front().executed == COMPLETED)) {
    for (uint32_t i = 0; i < MAX_INSTR_DESTINATIONS; i++) {
      if (ROB.front().destination_memory[i]) {
        ...
        data_packet.asid[0] = sq_it->asid[0];
        data_packet.asid[1] = sq_it->asid[1];
        data_packet.size = sq_it->size;
        data_packet.wdata = sq_it->wdata;

        auto result = L1D_bus.lower_level->add_wq(&data_packet);
        if (result != -2) {
          mem_data[cpu].write(sq_it->virtual_address, sq_it->wdata, sq_it->size);
