...
#define PRINT_INSTR_PC1  0xffffffffffffffff
#define PRINT_INSTR_PC2  0xffffffffffffffff
//#define PRINT_INSTR_PC1  0x120001664
//#define PRINT_INSTR_PC2  0x120001668
//#define PRINT_INSTR_PC1  0x120001670
//#define PRINT_INSTR_PC2  0x120001684
//#define PRINT_INSTR_PC1  0x1200012fc
...
#define nPRINT_WRITE_BACK_TRACE
#define nPRINT_EVICT_TRACE
...
if(/*NAME=="cpu0_L1D" && */(fill_mshr->v_address >> 6) == (0x4000a3b9c8 >> 6) && current_cycle > 122601075)
...
#ifdef PRINT_WRITE_BACK_TRACE
    if((handle_pkt.v_address >> 6) == (0x1200f7790 >> 6) && current_cycle > 98498360)
        cout << "[handle_writeback] " << NAME
             << ", instr: " << handle_pkt.instr_id 
             << ", cycles: " << current_cycle << hex 
             << ", vaddr: " << handle_pkt.v_address 
             << ", paddr: " << handle_pkt.address
             << ", hit: " << +(way < current_assoc)
...
if((handle_pkt.v_address >> 6) == (0x4000a3b9c8 >> 6) && current_cycle > 122601075)
...
if(handle_pkt.instr_id == PRINT_INSTR_ID || handle_pkt.ip  == PRINT_INSTR_PC1 || handle_pkt.ip  == PRINT_INSTR_PC2 ){
    if(NAME=="cpu0_L1D")
        cout << "\t[handle_read] " << NAME 
             << " instr: " << handle_pkt.instr_id 
             << ", pc: 0x" << handle_pkt.ip 
    ...
    if(NAME.rfind("L1D") != std::string::npos && handle_pkt.type == LOAD){
        collect_miss_info(handle_pkt, !(way < current_assoc));
    }
#endif
...
#ifdef PRINT_EVICT_TRACE
if(NAME=="cpu0_L1D" && (fill_block.v_address >> 6) == (0x1200e0cf0 >> 6))
    cout << "\t[evict     ] " << NAME 
         << ", instr: " << fill_block.instr_id 
         << ", cycles: " << current_cycle << hex
         << ", pc: 0x" << fill_block.ip
         << ", vaddr: " << fill_block.v_address 
         << ", paddr: " << fill_block.address << dec
         << dec << endl;
#endif
...
int CACHE::prefetch_line(uint64_t pf_addr, bool fill_this_level, uint64_t prefetch_metadata)
{
  pf_requested++;
    ...
    if((pf_addr >> 6) == (0x4000a3b9c8 >> 6) && current_cycle > 122601075)
...
void CACHE::va_translate_prefetches()
{
  ...
  if (VAPQ.has_ready()) {
    ...
    if((VAPQ.front().v_address >> 6) == (0x4000a3b9c8 >> 6) && current_cycle > 122601075){
