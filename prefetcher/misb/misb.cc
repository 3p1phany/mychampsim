...
IsbPrefetcher *misb_pref[NUM_CPUS];
...
#define MAX_ALLOWED_DEGREE PREFETCH_DISTANCE

#define ISB_FILL_LEVEL FILL_L2
//#define ISB_FILL_LEVEL FILL_LLC

bool misb_stride_enable = true;
extern IPT_L1 ipt[NUM_CPUS][IPT_NUM];

void CACHE::prefetcher_initialize() 
{
#ifdef ISB_ENABLE
    cout << NAME << " [ISB] Prefetcher, ";
#else
    cout << NAME << " [MISB] Prefetcher, ";
#endif

#if TEMPORAL_L1D == false
    misb_stride_enable = false;
...
//    conf[cpu]->count_off_chip_write_traffic = false;
    ...
    misb_pref[cpu] = new IsbPrefetcher(conf[cpu], cpu);

    misb_pref[cpu]->set_conf(conf[cpu]);
    misb_pref[cpu]->ideal_bloom_filter.clear();

#ifdef HYBRID
    bo_l2c_prefetcher_initialize();
#endif

    if(misb_stride_enable){
    ...
    cout << "Prefetching degree: " << conf[cpu]->degree << endl;
#ifdef BLOOM_ISB
    cout << "Bloom Filter On" << endl;
#else
    cout << "Bloom Filter Off" << endl;
#endif

    cout << "cpu id " << misb_pref[cpu]->get_cpu_id() << endl;
}
...
//#endif
    ...
    misb_pref[cpu]->metadata_read_requests.clear();
    misb_pref[cpu]->metadata_write_requests.clear();
     ...
     */
    misb_pref[cpu]->calculatePrefetch(addr, pc, cache_hit, prefetch_addr_list, MAX_ALLOWED_DEGREE);
    ...
    for(auto it=misb_pref[cpu]->metadata_read_requests.begin(); it != misb_pref[cpu]->metadata_read_requests.end(); it++)
    {
        //cout << "ISB: " << hex << misb_pref[cpu]->metadata_requests[i].phy_addr << " " << misb_pref[cpu]->metadata_requests[i].str_addr << dec << endl;
        get_metadata(*it);
    }

    for(auto it=misb_pref[cpu]->metadata_write_requests.begin(); it != misb_pref[cpu]->metadata_write_requests.end(); it++)
    {
        //cout << "ISB: " << hex << misb_pref[cpu]->metadata_requests[i].phy_addr << " " << misb_pref[cpu]->metadata_requests[i].str_addr << dec << endl;
        write_metadata(*it);
    }
...
{
    misb_pref[cpu]->prefetch_list.clear();
    misb_pref[cpu]->metadata_read_requests.clear();
    misb_pref[cpu]->metadata_write_requests.clear();


    misb_pref[cpu]->complete_metadata_req(meta_data_addr);


    uint64_t prefetched = 0;
    for (uint32_t i = 0; i < misb_pref[cpu]->prefetch_list.size(); ++i) {
        //cout << "[PF Buffer] " << hex << misb_pref[cpu]->prefetch_list[i] << dec << endl;
        int ret = prefetch_line(0, 0, misb_pref[cpu]->prefetch_list[i], FILL_L2, 0);
        if(ret)
        {
            prefetched++;
...
//{
//    misb_pref[cpu]->insert_metadata(phy_addr, str_addr, (off_chip_req_type_t)type);
//}
