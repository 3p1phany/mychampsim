...
void TriageOnchip::set_conf(TriageConfig *config) {
    ...
    max_assoc = config->max_assoc;

    cout << "Num Sets: " << num_sets
         << ", max_assoc: " << max_assoc
         << endl;
}
...
void TriageOnchip::update(uint64_t prev_addr, Metadata next_entry, uint64_t pc, bool update_repl) {
    if (use_dynamic_assoc) {
        if (repl->get_assoc() != target_assoc){
            // target_assoc = repl->get_assoc();
            uint32_t new_assoc = triage_unique_addr.size() / num_sets + 1;
            target_assoc = new_assoc > max_assoc ? max_assoc : new_assoc;
            wb_index = 0;
        }
    }
...
Metadata TriageOnchip::get_next_entry(uint64_t prev_addr, uint64_t pc, bool update_stats) {
    ...
    assert(set_id < num_sets);
    uint64_t line_offset = get_line_offset(prev_addr);
    uint64_t tag = prev_addr >> ONCHIP_LINE_SHIFT; // TODO: Triage tag include index?
    map<uint64_t, TriageOnchipEntry>& entry_map = entry_list[set_id];
...
uint32_t TriageOnchip::get_target_assoc()
{
    return target_assoc;
}
