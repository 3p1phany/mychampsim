...
for item in os.listdir(data_dir):
    ...
    for line in file.readlines():
        line_sp = line.split()
        if line.find("cpu0_L2C PREFETCH  REQUESTED:") != -1:
            dic["L2C_pref_total_num"] = int(line_sp[8])
            dic["L2C_pref_useless_num"] = int(line_sp[12])
            dic["L2C_pref_late_num"] = int(line_sp[14])
        if line.find("cpu0_L2C LOAD      ACCESS:") != -1:
            dic["L2C_pref_miss"] = int(line_sp[-1])
    result[name] = dic
    file.close()
...
for item in os.listdir(no_dir):
    ...
    for line in file.readlines():
        line_sp = line.split()
        if line.find("cpu0_L2C LOAD      ACCESS:") != -1:
            result[name]["L2C_origin_miss"] = int(line_sp[-1])
    file.close()
