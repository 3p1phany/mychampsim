...
with open('inc/cache_modules.inc', 'wt') as wfp:
    ...
    wfp.write('\n'.join('uint64_t {1}(uint64_t, uint64_t, uint8_t, bool, uint8_t, uint64_t);'.format(*p) for p in pref_ops if not p[0].startswith('CPU_REDIRECT')))
    wfp.write('\nuint64_t impl_prefetcher_cache_operate(uint64_t addr, uint64_t ip, uint8_t cache_hit, bool hit_pref, uint8_t type, uint64_t metadata_in)\n{\n    ')
    pref_ops = { (n, ('ooo_cpu[cpu]->{}(addr, cache_hit, (type == PREFETCH), metadata_in)' if n.startswith('CPU_REDIRECT') else '{}(addr, ip, cache_hit, hit_pref, type, metadata_in)').format(f)) for n,f in pref_ops } ## modify signature for redirect
