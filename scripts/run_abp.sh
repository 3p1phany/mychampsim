#!/usr/bin/env bash
# ABP (Access Based Predictor) one-click build & run script
# Usage:
#   TRACE_ROOT=/path/to/traces ./scripts/run_abp.sh
#
...
#   LABEL       - result label (default: ABP_1c)
...
if [[ "$SKIP_BUILD" != "1" ]]; then
    ...
    echo "========================================"
    echo "[2/3] Generating Makefile & building ChampSim (ABP config)..."
    echo "========================================"
    python3 config.sh champsim_config_ABP.json
    make -j8
