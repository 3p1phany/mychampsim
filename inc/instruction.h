...
#define COLLECT_CACHE_LOAD_INFO
...
extern uint64_t reg500_inst_num;
...
extern uint64_t invalid_format_num;
...
#ifdef ADDR_CHECK
    if(op_is_load(op)){
        bool cond = !(instr.source_memory[0] & 0xffffff0000000000 || instr.source_memory[0] == 0);        
        if(!cond){
            prinf_info();
            fflush(stdout);
            lsassertm(cond, "Load Address Exception! %llx\n", instr.source_memory[0]);
            exit(1);
        }
    }

    if(op_is_store(op)){
        bool cond = !(instr.destination_memory[0] & 0xffffff0000000000 || instr.destination_memory[0] == 0);
        lsassertm(cond, "Store Address Exception! %llx\n", instr.destination_memory[0]);
    }
#endif
