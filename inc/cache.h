...
public:
  ...
  // prefetch stats
  uint64_t pf_requested = 0, pf_issued = 0, pf_miss_issued = 0, pf_useful = 0, pf_useless = 0, pf_fill = 0, pf_late = 0;
  ...
  uint64_t sim_read_access[NUM_CPUS][NUM_TYPES] = {}, sim_read_hit[NUM_CPUS][NUM_TYPES] = {}, sim_read_miss[NUM_CPUS][NUM_TYPES] = {},
           roi_read_access[NUM_CPUS][NUM_TYPES] = {}, roi_read_hit[NUM_CPUS][NUM_TYPES] = {}, roi_read_miss[NUM_CPUS][NUM_TYPES] = {};

  std::unordered_map<uint64_t, load_miss_info_t> sim_load_info[NUM_CPUS];

  uint64_t MSHR_FULL = 0, VAPQ_FULL = 0;
  ...
  int add_pq_assign_lat(PACKET* packet, long long int latency);

  void return_data(PACKET* packet) override;
  void operate() override;
  uint32_t operate_writes();
  uint32_t operate_reads();
...
#include "cache_modules.inc"
  ...
  // constructor
  CACHE(std::string v1, uint32_t cpu_id, double freq_scale, unsigned fill_level, uint32_t v2, int v3, uint32_t v5, uint32_t v6, uint32_t v7, uint32_t v8, uint32_t hit_lat,
        ...
        unsigned pref_act_mask, MemoryRequestConsumer* ll, pref_t pref, repl_t repl, bool packet_can_merge)
      : champsim::operable(freq_scale), MemoryRequestConsumer(fill_level), MemoryRequestProducer(ll), cpu(cpu_id), NAME(v1), NUM_SET(v2), NUM_WAY(v3), WQ_SIZE(v5),
        ...
        repl_type(repl), pref_type(pref), packet_can_merge(packet_can_merge)
  {
